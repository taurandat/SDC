...
def gradient_abs_value_mask(image, sobel_kernel=3, axis='x', threshold=(0, 255)):
    ...
    if axis == 'x':
        sobel = np.absolute(
            cv2.Sobel(image, cv2.CV_64F, 1, 0, ksize=sobel_kernel))
    if axis == 'y':
        sobel = np.absolute(
            cv2.Sobel(image, cv2.CV_64F, 0, 1, ksize=sobel_kernel))

    # Scale to 8-bit (0 - 255) then convert to type = np.uint8
    sobel = np.uint8(255 * sobel / np.max(sobel))

    # Create a mask of 1's where the scaled gradient magnitude is > thresh_min
    # and < thresh_max
...
def gradient_direction_mask(image, sobel_kernel=3, threshold=(0, np.pi / 2)):
    ...
    # Take the absolute value of the x and y gradients and calculate the
...
def get_edges(image, separate_channels=False):
    ...
    gradient_x = gradient_abs_value_mask(s_channel,
                                         axis='x',
                                         sobel_kernel=3,
                                         threshold=(20, 100))
    gradient_y = gradient_abs_value_mask(s_channel,
                                         axis='y',
                                         sobel_kernel=3,
                                         threshold=(20, 100))
    magnitude = gradient_magnitude_mask(s_channel,
                                        sobel_kernel=3,
                                        threshold=(20, 100))
    direction = gradient_direction_mask(s_channel,
                                        sobel_kernel=3,
                                        threshold=(0.7, 1.3))
